...
def cleanWorkspace (items) {
  for (item in items) {
    if (item.class.canonicalName == null) { 
      ...
      for (subitem in item.items) {
        if (subitem.class.canonicalName == "org.jenkinsci.plugins.workflow.job.WorkflowJob") {
          ...
          processBuildsForWorkflowItem(subitem, true);
        }
        else {
    ...
    else if (item.class.canonicalName != "hudson.model.ExternalJob") {
      	println("Processing item " + item.name + " (" + item.class.canonicalName+")")
